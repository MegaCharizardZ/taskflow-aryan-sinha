...
public interface ProjectRepository extends JpaRepository<Project, UUID> {
    ...
    /**
     * Projects the user owns, is assigned to a task in, or has created a task in.
     */
    @Query("""
            SELECT DISTINCT p FROM Project p
            WHERE p.owner.id = :userId
               OR p.id IN (SELECT t.project.id FROM Task t WHERE t.assignee.id = :userId)
               OR p.id IN (SELECT t.project.id FROM Task t WHERE t.createdBy.id = :userId)
            ORDER BY p.createdAt DESC
            """)
    List<Project> findVisibleToUser(@Param("userId") UUID userId);

    /**
     * Owner, assignee on at least one task, or creator of at least one task in this project.
     */
    @Query("""
            ...
            WHERE p.id = :projectId
              AND (p.owner.id = :userId
                   ...
                   OR EXISTS (SELECT 1 FROM Task t WHERE t.project.id = p.id AND t.createdBy.id = :userId))
            """)
